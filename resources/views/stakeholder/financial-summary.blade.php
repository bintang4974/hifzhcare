    ...
    <div class="space-y-6">
        ...
        <section class="rounded-3xl border border-slate-200 bg-white shadow-sm p-6 lg:p-8">
            ...
            <section class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
                <div
                    class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-500 to-teal-600 p-6 text-white shadow-lg">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-3 text-3xl font-bold leading-none">Rp
                                {{ number_format($financial['total_revenue'], 0, ',', '.') }}</p>
                ...
                <div
                    class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-sky-500 to-blue-600 p-6 text-white shadow-lg">
                    <div class="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20">
                        <i class="fas fa-percent text-xl"></i>
                    </div>
                    <p class="text-sm font-medium text-sky-50">Platform Fee ({{ $financial['platform_percentage'] }}%)</p>
                    <p class="mt-3 text-3xl font-bold leading-none">Rp
                        {{ number_format($financial['platform_fee'], 0, ',', '.') }}</p>
                    <p class="mt-3 text-xs text-sky-50/90">Fee untuk platform</p>
                </div>

                <div
                    class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-500 to-purple-600 p-6 text-white shadow-lg">
                    <div class="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20">
                        <i class="fas fa-mosque text-xl"></i>
                    </div>
                    <p class="text-sm font-medium text-violet-50">Dana Pesantren ({{ $financial['pesantren_percentage'] }}%)
                    </p>
                    <p class="mt-3 text-3xl font-bold leading-none">Rp
                        {{ number_format($financial['pesantren_share'], 0, ',', '.') }}</p>
                    <p class="mt-3 text-xs text-violet-50/90">Bagian pesantren</p>
                </div>

                <div
                    class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-orange-500 to-amber-600 p-6 text-white shadow-lg">
                    <div class="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/20">
                        <i class="fas fa-hand-holding-usd text-xl"></i>
                    </div>
                    <p class="text-sm font-medium text-orange-50">Dana Ustadz ({{ $financial['ustadz_percentage'] }}%)</p>
                    <p class="mt-3 text-3xl font-bold leading-none">Rp
                        {{ number_format($financial['ustadz_total'], 0, ',', '.') }}</p>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div class="rounded-2xl border border-orange-200 bg-orange-50 p-4">
                                ...
                                <p class="mt-1 text-sm text-orange-700">Rp
                                    {{ number_format($financial['ustadz_total'], 0, ',', '.') }}</p>
                            </div>
                            <div class="rounded-2xl border border-violet-200 bg-violet-50 p-4">
                                ...
                                <p class="mt-2 text-lg font-bold text-slate-900">{{ $financial['pesantren_percentage'] }}%
                                </p>
                                <p class="mt-1 text-sm text-violet-700">Rp
                                    {{ number_format($financial['pesantren_share'], 0, ',', '.') }}</p>
                            </div>
                            <div class="rounded-2xl border border-sky-200 bg-sky-50 p-4">
                                <p class="text-xs font-medium text-sky-700">Platform Fee</p>
                                <p class="mt-2 text-lg font-bold text-slate-900">{{ $financial['platform_percentage'] }}%
                                </p>
                                <p class="mt-1 text-sm text-sky-700">Rp
                                    {{ number_format($financial['platform_fee'], 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-6 grid grid-cols-1 gap-4 border-t border-slate-200 pt-6 sm:grid-cols-3">
                    <div class="rounded-2xl bg-blue-50 p-5 text-center">
                        ...
                        <p class="text-2xl font-bold text-blue-600">Rp
                            {{ number_format($financial['monthly_avg'], 0, ',', '.') }}</p>
                    </div>
                    <div class="rounded-2xl bg-emerald-50 p-5 text-center">
                        ...
                        <p class="text-2xl font-bold text-emerald-600">Rp
                            {{ number_format($financial['highest_month'], 0, ',', '.') }}</p>
                    </div>
                    <div class="rounded-2xl bg-slate-50 p-5 text-center">
                        <p class="text-sm text-slate-500 mb-1">Pertumbuhan</p>
                        <p
                            class="text-2xl font-bold {{ $financial['revenue_growth'] >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
                ...
                <div class="rounded-3xl border border-slate-200 bg-white p-6 lg:p-8 shadow-sm">
                    ...
                    <div class="space-y-4">
                        @foreach ($donation_status as $status)
                            <div
                                class="flex items-center justify-between rounded-2xl border border-{{ $status['color'] }}-200 bg-{{ $status['color'] }}-50 p-4">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="flex h-11 w-11 items-center justify-center rounded-2xl bg-{{ $status['color'] }}-100">
                                ...
                                <div class="text-right">
                                    <p class="text-lg font-bold text-{{ $status['color'] }}-600">Rp
                                        {{ number_format($status['amount'], 0, ',', '.') }}</p>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div class="rounded-2xl bg-violet-50 p-5">
                                ...
                                <p class="text-xl font-bold text-violet-600">Rp
                                    {{ number_format($financial['avg_donation'], 0, ',', '.') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="rounded-2xl bg-orange-50 p-5">
                            ...
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <span class="text-lg font-semibold text-slate-900">Rp
                                    {{ number_format($financial['pending_amount'], 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                @foreach ($top_contributors as $index => $contributor)
                                    <tr class="hover:bg-slate-50/80">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-pink-400 to-rose-500 font-bold text-white">
                                        ...
                                        <td class="px-4 py-4 text-center whitespace-nowrap">
                                            <p class="text-lg font-bold text-emerald-600">Rp
                                                {{ number_format($contributor['total_amount'], 0, ',', '.') }}</p>
                                        </td>
                                        <td class="px-4 py-4 text-center whitespace-nowrap">
                                            <span
                                                class="text-lg font-bold text-blue-600">{{ $contributor['transaction_count'] }}</span>
                                        </td>
                                        <td class="px-4 py-4 text-center whitespace-nowrap">
                                            <p class="text-sm font-semibold text-slate-900">Rp
                                                {{ number_format($contributor['avg_amount'], 0, ',', '.') }}</p>
                                        </td>
                                        <td class="px-4 py-4 whitespace-nowrap">
                        ...
                        <div class="rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-50 to-purple-50 p-5">
                            <div class="mb-4 flex items-center justify-between gap-4">
                                <div class="flex items-center gap-3">
                                    <span class="text-2xl">
                                        ...
                                        @else
                                            <span
                                                class="flex h-8 w-8 items-center justify-center rounded-full bg-indigo-100 text-sm font-bold text-indigo-600">{{ $index + 1 }}</span>
                                        @endif
                                    </span>
                                    <div>
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                                <div class="rounded-xl bg-white/80 p-3 text-center">
                                    ...
                                    <p class="mt-1 text-sm font-bold text-emerald-600">Rp
                                        {{ number_format($ustadz['total_received'], 0, ',', '.') }}</p>
                                </div>
                                <div class="rounded-xl bg-white/80 p-3 text-center">
                                ...
                                <div class="rounded-xl bg-white/80 p-3 text-center">
                                    ...
                                    <p class="mt-1 text-sm font-bold text-violet-600">Rp
                                        {{ number_format($ustadz['disbursed'], 0, ',', '.') }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            <section class="grid grid-cols-1 gap-5 md:grid-cols-3">
                <div
                    class="rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-teal-50 p-6 shadow-sm">
                    ...
                    <div class="mt-4 h-3 w-full rounded-full bg-slate-200">
                        <div class="h-3 rounded-full bg-emerald-500" style="width: {{ $financial['cash_flow_score'] }}%">
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-3xl border border-sky-200 bg-gradient-to-br from-sky-50 to-indigo-50 p-6 shadow-sm">
                    ...
                    <div class="mt-4 h-3 w-full rounded-full bg-slate-200">
                        <div class="h-3 rounded-full bg-sky-500" style="width: {{ $financial['collection_rate'] }}%">
                        </div>
                    </div>
                </div>

                <div
                    class="rounded-3xl border border-violet-200 bg-gradient-to-br from-violet-50 to-pink-50 p-6 shadow-sm">
                    <div class="mb-4 flex items-center justify-between">
                    ...
                    <div class="mt-4 h-3 w-full rounded-full bg-slate-200">
                        <div class="h-3 rounded-full bg-violet-500"
                            style="width: {{ $financial['disbursement_rate'] }}%"></div>
                    </div>
                </div>
            </section>
    </div>
